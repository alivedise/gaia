'use strict';
/* globals applications, VersionHelper, FtuPing, System, BaseModule */
  ...
  BaseModule.create(FtuLauncher, {
      ...
      switch (evt.type) {
        case 'appopened':
          if (evt.detail.origin == this._ftuOrigin && this._isRunningFirstTime) {
            this.publish('open');
            this._queuingRequests.forEach(function(resolve) {
              resolve();
            });
          }
          break;
    ...
    launch: function fl_launch() {
      ...
      req.onerror = function() {
        self.warn('Couldn\'t get the ftu manifestURL.\n');
        self.skip();
      };
    },
    ...
    retrieve: function fl_retrieve() {
      ...
      VersionHelper.getVersionInfo().then(function(versionInfo) {
        ...
        self.warn('VersionHelper failed to lookup version settings, skipping.\n');
        self.skip();
      });
    }
  };
