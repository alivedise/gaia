...
(function(window) {
  ...
  AttentionWindow.prototype.view = function attw_view() {
    ...
    return '<div class="appWindow attentionWindow' +
            (this.hasTelephonyPermission() ? ' telephonyWindow ' : '') +
  ...
  AttentionWindow.prototype.hasTelephonyPermission =
    function attw_hasAttentionPermission() {
      if (typeof(this._hasTelephonyPermission) !== 'undefined') {
        return this._hasTelephonyPermission;
      }
      var mozPerms = navigator.mozPermissionSettings;
      if (!mozPerms)
        return false;

      var value = mozPerms.get('telephony',
        this.manifestURL, this.origin, false);
      this._hasTelephonyPermission = (value === 'allow');
